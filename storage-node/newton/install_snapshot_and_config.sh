...
if [[ "$OPTION" == "1" ]]; then
    ...
    sed -i "s|miner_key = \".*\"|miner_key = \"$MINER_KEY\"|g" "$HOME/0g-storage-node/run/config.toml"
...
EOF
    ...
    SNAPSHOT="snapshot_standard_astrostake_2025-03-15.tar.lz4"
    if [ -f "$SNAPSHOT" ]; then
    ...
    pv -pterb "$SNAPSHOT" | lz4 -c -d | tar -x -C $HOME/0g-storage-node/run
