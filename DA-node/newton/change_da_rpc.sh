...
echo "==================================="
echo " DA Node RPC Updater by AstroStake "
echo "==================================="
echo ""
...
CURRENT_RPC=$(grep -oP '(?<=^eth_rpc_endpoint = ")[^"]+' "$CONFIG_FILE")
...
# Update konfigurasi
sed -i "s|^eth_rpc_endpoint = .*|eth_rpc_endpoint = \"$NEW_RPC\"|" "$CONFIG_FILE"

# Restart service zgs
echo ""
echo "Restarting da-node service..."
